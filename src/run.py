...
class ParagraphVectorModel():
    def __init__(self, wordEmbeddings, filesCount, fileEmbeddingSize, windowSize, negative):
        ...
        fileIndices = input[:,0:1]
        wordIndices = input[:,1:-1]
        targetWordIndices = input[:,-1:]

        files = self.fileEmbeddings[fileIndices]
        words = self.wordEmbeddings[wordIndices]
        embeddings = T.concatenate([files, words], axis=1)
        contextEmbeddings = T.reshape(embeddings, (embeddings.shape[0], embeddings.shape[1] * embeddings.shape[2]))

        random = RandomStreams(0)
        negativeIndices = random.random_integers((targetWordIndices.shape[0], negative), 0, wordsCount - 1, dtype='int32')
        indices = T.concatenate([targetWordIndices, negativeIndices], axis=1)

        probabilities, updates = theano.map(
            fn=lambda e, i, weight, bias: T.nnet.softmax(T.dot(e, weight[:,i]) + bias[i]),
            sequences=[contextEmbeddings, indices],
            non_sequences=[self.weights, self.biases]
        )

        cost = -T.mean(T.log(probabilities)[T.arange(probabilities.shape[0]), 0])
        ...
        self.trainModel = theano.function(
            ...
            updates=updates,
            givens={
                input: self.contexts[miniBatchIndex * miniBatchSize: (miniBatchIndex + 1) * miniBatchSize]
            })
    ...
    def train(self, contexts, miniBatchSize, learningRate):
        ...
        miniBatchesCount = contextsCount / miniBatchSize

        for miniBatchIndex in xrange(0, miniBatchesCount):
            result = self.trainModel(miniBatchIndex, miniBatchSize, learningRate)

            log.progress('Training model: {0:.3f}%. Result {1}.',
                miniBatchIndex + 1, miniBatchesCount, result)
...
def trainFileEmbeddings(fileIndexMapFilePath, wordEmbeddingsFilePath, contextsPath,
                        fileEmbeddingsPath, fileEmbeddingSize, epochs, superBatchSize, miniBatchSize, learningRate, negative):
    ...
    model = ParagraphVectorModel(wordEmbeddings, filesCount, fileEmbeddingSize, windowSize, negative)
